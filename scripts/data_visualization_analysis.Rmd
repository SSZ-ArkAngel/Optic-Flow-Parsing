...
Same.Opposite %>%
  ...
  ggplot(aes(x = Same, y = Opposite, label = row.names)) + # EDIT
...
top.bottom %>%
  ...
  ggplot(aes(x = FT, y = FB, label = subject)) +
...
for(i in unique(bootstrap_data$subject)) {
  ...
  bottom = as.vector(Bottom.top - Bottom.bottom, mode = 'numeric')
...
Local %>%
  ...
  ggplot(aes(x = Local_top, y = Local_bottom, label = subject)) +
  geom_point(colour = Local$subject, size = 3) +
  
  # ADDED
  geom_text(aes(label = subject, hjust = -0.5, vjust = -0.5), check_overlap = TRUE) +

  geom_errorbar(aes(ymin = local_data[local_data$local_effect == "bottom" , ]$lower,
                    ymax = local_data[local_data$local_effect == "bottom" , ]$upper)) +

  geom_errorbarh(aes(xmin = local_data[local_data$local_effect == "top" , ]$lower,
                     xmax = local_data[local_data$local_effect == "top" , ]$upper)) +
...
Control %>%
  ...
  ggplot(aes(x = Control.top, y = Control.bottom, label = subject)) +
  geom_point(colour = Control$subject, size = 3) +
    
  # ADDED
  geom_text(aes(label = subject, hjust = 0, vjust = -2), check_overlap = TRUE) +
  
  geom_errorbar(aes(ymin = CI[CI$condition == "Control" & CI$probe_index == "bottom", ]$lower,
                    ymax = CI[CI$condition == "Control" & CI$probe_index == "bottom", ]$upper)) +
  
  geom_errorbarh(aes(xmin = CI[CI$condition == "Control" & CI$probe_index == "top", ]$lower,
                     xmax = CI[CI$condition == "Control" & CI$probe_index == "top", ]$upper)) +
