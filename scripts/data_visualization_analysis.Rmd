...
```{r Implementing exclusion criteria based on RT}
...
Organize bootstrapped samples.
```{r}
boo <- read.csv(paste(dirname(getwd()), "/bootstrapped data/bootstrap_data.csv", sep = ""))
...
Top.Bottom %<>%
  # mutate(
  #   Top = 
  #     (CI[CI$condition == "Top" & CI$probe_index == "top", ]$median_alpha + CI[CI$condition == "Top" & CI$probe_index == "bottom", ]$median_alpha) / 2,
  #   
  #   Bottom =
  #     (CI[(CI$condition == "Bottom" & CI$probe_index == "top"), ]$median_alpha + CI[CI$condition == "Bottom" & CI$probe_index == "bottom", ]$median_alpha) / 2
  # ) %>%
  ...
  # Add title
  ggtitle("Average Best Fitting Gain for Optic Flow in Top vs. Bottom", subtitle = paste("with bootstrapping. N =", n_subjects)) +
...
```{r Top-bottom vs. Bottom-top *with CIs*}
...
Opposite %>%
  ...
  # Add axis labels
  xlab("Gain when flow-Bottom probe-top") +    
  ylab("Gain when flow-Top probe-bottom") +
