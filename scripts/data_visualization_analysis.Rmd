...
# # save ggplot
# ggsave(filename = paste(paste("Subject", select_subject), "all.png"),
...
# # save ggplot
# ggsave(filename = "Same vs. Opposite.png",
#        plot = last_plot(), path = "[file path here]")
```
...
# # save ggplot
# ggsave(filename = "Full-top vs. Full-bottom.png",
#        plot = last_plot(), path = "[file path here]")
```
...
# # save ggplot
# ggsave(filename = "OF Top vs. OF Bottom.png",
#        plot = last_plot(), path = "[file path here]")
```
...
# # save ggplot
# ggsave(filename = "Top-top vs. Bottom-bottom.png",
#        plot = last_plot(), path = "[file path here]")
```
...
## Organize CI data.
```{r}
# CI <- read.csv(paste(dirname(getwd()), "/bootstrapped data/CI.csv", sep = "")) # CI with constrained alpha
CI <- read.csv(paste(dirname(getwd()), "/bootstrapped data/CI_nonconstrained.csv", sep = "")) # CI with non-constrained alpha
...
```{r}
# bootstrap_samples <- read.csv(paste(dirname(getwd()), "/bootstrapped data/bootstrap_data.csv", sep = "")) # constrained alpha
bootstrap_samples <- read.csv(paste(dirname(getwd()), "/bootstrapped data/bootstrap_data_nonconstrained.csv", sep = "")) # non-constrained alpha
...
## Prediction plots (N=1) *with CI labels*

Add bootstrapped median and CI to data frame (dummy method; used for now :) ).
```{r}
data_CI_tmp <- data
data_CI = data_CI_tmp %<>%
  
  filter(subject == 1, condition == "Full", probe_index == "top") %>%
  
  mutate(
    bootstrap_lower = CI[CI$subject == 1 & CI$condition == "Full" & CI$probe_index == "top", ]$lower,
    bootstrap_median = CI[CI$subject == 1 & CI$condition == "Full" & CI$probe_index == "top", ]$median_alpha,
    bootstrap_upper = CI[CI$subject == 1 & CI$condition == "Full" & CI$probe_index == "top", ]$upper
  )

for (i in unique(data$subject)) {
  for (j in unique(data$condition)) {
    for (k in unique(data$probe_index)) {
      
      if (i == 1 & j == "Full" & k == "top") {

      } else {
        # print(paste(i, paste(j, k))) # DEBUG
        
        data_CI_tmp <- data
        
        tmp = data_CI_tmp %<>%
          filter(subject == i, condition == j, probe_index == k) %>%
          mutate(
            bootstrap_lower = CI[CI$subject == i & CI$condition == j & CI$probe_index == k, ]$lower,
            bootstrap_median = CI[CI$subject == i & CI$condition == j & CI$probe_index == k, ]$median_alpha,
            bootstrap_upper = CI[CI$subject == i & CI$condition == j & CI$probe_index == k, ]$upper
          )
        
        data_CI = bind_rows(data_CI, tmp)
      }
...
data_CI %>%
  ...
  geom_abline() + # identity line plotted *first*
  ...
  geom_text(
    data = ~ .x %>%
      select(alpha) %>%
      distinct(),
    aes(label = paste("Gain:", round(alpha, digits = 2))),
    x = 0,
    y = 360,
    hjust = 0,
    show.legend = FALSE
  ...
  # plot CI (bootstrapped data)
  geom_text(
    data = ~ .x %>%
      select(bootstrap_lower, bootstrap_upper) %>%
      distinct(),
    aes(label = paste(paste(paste(paste("CI: [", round(bootstrap_lower, digits = 2)), ","), round(bootstrap_upper, digits = 2)), "]")),
    x = 0,
    y = 320,
    hjust = 0,
    show.legend = FALSE
  ...
  # plots of 1 subject for each unique condition
  facet_grid(Probe ~ Flow, labeller = label_both) +
...
for (i in 1:nrow(CI)) {
  ...
  rownames(CI)[i] <- paste(CI$subject[i], paste(substr(CI$condition[i],1,1), substr(CI$probe_index[i],1,1), sep = "-"), sep = ", ")
}
```
...
Same.Opposite %<>%
  mutate(
    ...
    # retain info from CI dataframe
    subject = CI[(CI$condition == "Top" & CI$probe_index == "top") | (CI$condition == "Bottom" & CI$probe_index == "bottom"), ]$subject,
    row.names = rownames(CI[(CI$condition == "Top" & CI$probe_index == "top") | (CI$condition == "Bottom" & CI$probe_index == "bottom"), ])
  )

Same.Opposite %>%
  
  ggplot(aes(x = Opposite, y = Same, label = row.names)) + # EDIT
  
  geom_abline() + # identity line plotted *first*
  geom_hline(yintercept = 0, lty = 2) +
  geom_vline(xintercept = 0, lty = 2) +
  
  geom_point(colour = Same.Opposite$subject, size = 2.5) +
  
  # ADDED
  geom_text(aes(label = row.names, hjust = 0.5, vjust = -1), check_overlap = TRUE) +
  
  geom_errorbar(aes(ymin = CI[(CI$condition == "Top" & CI$probe_index == "top") | (CI$condition == "Bottom" & CI$probe_index == "bottom"), ]$lower,
                    ymax = CI[(CI$condition == "Top" & CI$probe_index == "top") | (CI$condition == "Bottom" & CI$probe_index == "bottom"), ]$upper)) +
  
  geom_errorbarh(aes(xmin = CI[(CI$condition == "Top" & CI$probe_index == "bottom") | (CI$condition == "Bottom" & CI$probe_index == "top"), ]$lower,
                     xmax = CI[(CI$condition == "Top" & CI$probe_index == "bottom") | (CI$condition == "Bottom" & CI$probe_index == "top"), ]$upper)) +
  ...
  # Add axis labels
  xlab(TeX("Gain F_{Bottom} P_{Top} or F_{Top} P_{Bottom}")) +    
  ylab(TeX("Gain F_{Bottom} P_{Bottom} or F_{Top} P_{Top}")) +
  
  # Add title
  ggtitle("Relative Strength of Flow Parsing for Same vs. Opposite") +
...
# # save ggplot
# ggsave(filename = "Same vs. Opposite with CIs.png",
#        plot = last_plot(), path = "[file path here]")
```

### Full-top vs. Full-bottom *with CIs*
```{r}
top.bottom <- tibble(.rows = n_subjects) # empty table n_subjects x n_conditions
...
top.bottom %<>%
  mutate(
    ...
    # retain info from CI dataframe
    subject = CI[(CI$condition == "Full" & CI$probe_index == "top"), ]$subject,
  )

top.bottom %>%

  ggplot(aes(x = FB, y = FT, label = subject)) +
  
  geom_abline() + # identity line plotted *first*
  
  geom_point(colour = top.bottom$subject, size = 3) +
  
  # ADDED
  geom_text(aes(label = subject, hjust = 2, vjust = 1.5), check_overlap = FALSE) +
  
  geom_errorbar(aes(ymin = CI[CI$condition == "Full" & CI$probe_index == "top", ]$lower, ymax = CI[CI$condition == "Full" & CI$probe_index == "top", ]$upper)) +
  
  geom_errorbarh(aes(xmin = CI[CI$condition == "Full" & CI$probe_index == "bottom", ]$lower, xmax = CI[CI$condition == "Full" & CI$probe_index == "bottom", ]$upper)) +
  ...
  # Add axis labels
  xlab(TeX("Gain F_{Full} P_{Bottom}")) +    
  ylab(TeX("Gain F_{Full} P_{Top}")) +
  
  # Add title
  ggtitle("Relative Strength of Flow Parsing between Vertical Hemifields") +
...
# # save ggplot
# ggsave(filename = "Full-top vs. Full-bottom with CIs.png",
#        plot = last_plot(), path = "[file path here]")
```
...
Top.Bottom %<>%
  ...
  mutate(
    Top =
      CI[(CI$condition == "Top" & CI$probe_index == "top") | (CI$condition == "Top" & CI$probe_index == "bottom"), ]$median_alpha,

    Bottom =
      CI[(CI$condition == "Bottom" & CI$probe_index == "top") | (CI$condition == "Bottom" & CI$probe_index == "bottom"), ]$median_alpha,
    
    # retain info from CI dataframe
    subject = CI[(CI$condition == "Top" & CI$probe_index == "top") | (CI$condition == "Top" & CI$probe_index == "bottom"), ]$subject,
  )

Top.Bottom %>%
  
  ggplot(aes(x = Bottom, y = Top, 
  ...
  geom_errorbar(aes(ymin = CI[(CI$condition == "Top" & CI$probe_index == "top") | CI$condition == "Top" & CI$probe_index == "bottom", ]$lower,
                    ymax = CI[(CI$condition == "Top" & CI$probe_index == "top") | CI$condition == "Top" & CI$probe_index == "bottom", ]$upper)) +

  geom_errorbarh(aes(xmin = CI[(CI$condition == "Bottom" & CI$probe_index == "top") | (CI$condition == "Bottom" & CI$probe_index == "bottom"), ]$lower,
                     xmax = CI[(CI$condition == "Bottom" & CI$probe_index == "top") | (CI$condition == "Bottom" & CI$probe_index == "bottom"), ]$upper)) +
  ...
  # Add axis labels
  xlab("Gain when optic flow in lower hemifield") +    
  ylab("Gain when optic flow in upper hemifield") +
  
  # Add title
  ggtitle("Best Fitting Gain for Optic Flow in Top vs. Bottom") +
...
# # save ggplot
# ggsave(filename = "OF Top vs. OF Bottom with CIs.png",
#        plot = last_plot(), path = "[file path here]")
```
...
Same.comparison %<>%
  mutate(
    ...
    # retain info from CI dataframe
    subject = CI[(CI$condition == "Top" & CI$probe_index == "top"), ]$subject,
  )

Same.comparison %>%
  
  ggplot(aes(x = Same.Bottom, y = Same.Top, label = subject)) +
  geom_point(colour = Same.comparison$subject, size = 3) +
  
  # ADDED
  geom_text(aes(label = subject, hjust = 0, vjust = -2), check_overlap = TRUE) +
  ...
  # Add title
  ggtitle("Accuracy of Report for Same Flow/probe Locations") +
...
# # save ggplot
# ggsave(filename = "Top-top vs. Bottom-bottom with CIs.png",
#        plot = last_plot(), path = "[file path here]")
```
...
Opposite %<>%
  mutate(
    ...
    # retain info from CI dataframe
    subject = CI[(CI$condition == "Top" & CI$probe_index == "bottom"), ]$subject
  )

Opposite %>%
  
  ggplot(aes(x = Bottom_top, y = Top_bottom, label = subject)) +
  
  geom_abline() + # identity line plotted *first*
  geom_hline(yintercept = 0, lty = 2) +
  geom_vline(xintercept = 0, lty = 2) +
  
  geom_point(colour = Opposite$subject, size = 3) +
  ...
  # Add axis labels
  xlab(TeX("Gain F_{Top} P_{Bottom}")) +    
  ylab(TeX("Gain F_{Bottom} P_{Top}")) +
  
  # Add title
  ggtitle("Relative Strength of Flow Parsing for Opposite Conditions") +
...
# # save ggplot
# ggsave(filename = "Top-bottom vs. Bottom-top with CIs.png",
#        plot = last_plot(), path = "[file path here]")
```

### Local top vs. Local bottom *with CIs* [NEW!]

Organize data frame #1
```{r}
local_data <- tibble(subject = numeric(0), local_effect = character(0), lower = numeric(0), median = numeric(0), upper = numeric(0))

for(i in unique(bootstrap_data$subject)) {

  # Top-Top - Top-bottom
  Top.top = bootstrap_data %>%
    filter(subject == i, condition == "Top", probe_index == "top") %>%
    subset(select = c(7:1006))
  
  Top.bottom = bootstrap_data %>%
    filter(subject == i, condition == "Top", probe_index == "bottom") %>%
    subset(select = c(7:1006))
  
  top = as.vector(Top.top - Top.bottom, mode = 'numeric')
  top.lower = quantile(top, 0.025)
  top.median = quantile(top, 0.5)
  top.upper = quantile(top, 0.975)
  
  local_data %<>%
    add_row(subject = i, local_effect = "top", lower = top.lower, median = top.median, upper = top.upper)
  
  # Bottom-bottom - Bottom-top
  Bottom.top = bootstrap_data %>%
    filter(subject == i, condition == "Bottom", probe_index == "top") %>%
    subset(select = c(7:1006))
  
  Bottom.bottom = bootstrap_data %>%
    filter(subject == i, condition == "Bottom", probe_index == "bottom") %>%
    subset(select = c(7:1006))
  
  bottom = as.vector(Bottom.bottom - Bottom.top, mode = 'numeric')
  bottom.lower = quantile(bottom, 0.025)
  bottom.median = quantile(bottom, 0.5)
  bottom.upper = quantile(bottom, 0.975)
  
  local_data %<>%
    add_row(subject = i, local_effect = "bottom", lower = bottom.lower, median = bottom.median, upper = bottom.upper)
}
```

Organize data frame #2
```{r}
local_data <- tibble(subject = numeric(0), local_effect = character(0), lower = numeric(0), median = numeric(0), upper = numeric(0))
...
for(i in unique(bootstrap_data$subject)) {
  ...
  # Local top
  Full.top = bootstrap_data %>%
    filter(subject == i, condition == "Full", probe_index == "top") %>%
    subset(select = c(7:1006))
  ...
  Full.bottom = bootstrap_data %>%
    filter(subject == i, condition == "Full", probe_index == "bottom") %>%
    subset(select = c(7:1006))
  ...
  top = as.vector((Full.top - Bottom.top) - (Full.bottom - Bottom.bottom), mode = 'numeric')
  top.lower = quantile(top, 0.025)
  top.median = quantile(top, 0.5)
  top.upper = quantile(top, 0.975)
  ...
  bottom = as.vector((Full.bottom - Top.bottom) - (Full.top - Top.top), mode = 'numeric')
  bottom.lower = quantile(bottom, 0.025)
  bottom.median = quantile(bottom, 0.5)
  bottom.upper = quantile(bottom, 0.975)
...
Plotting
```{r}
Local <- tibble(.rows = n_subjects) # empty table n_subjects x n_conditions

Local %<>%
  mutate(
    Local_top = local_data[local_data$local_effect == "top", ]$median,
    Local_bottom = local_data[local_data$local_effect == "bottom", ]$median,
    
    # retain info from local_data dataframe
    subject = local_data[local_data$local_effect == "top", ]$subject
  )

Local %>%

  ggplot(aes(x = Local_bottom, y = Local_top, label = subject)) +
    
  geom_abline() + # identity line plotted *first*
  # geom_hline(yintercept = 0, lty = 2) +
  # geom_vline(xintercept = 0, lty = 2) +
  
  geom_point(colour = Local$subject, size = 2) +
  
  # ADDED
  geom_text(aes(label = subject, hjust = -0.5, vjust = -0.5), check_overlap = TRUE) +

  geom_errorbar(aes(ymin = local_data[local_data$local_effect == "top" , ]$lower,
                    ymax = local_data[local_data$local_effect == "top" , ]$upper)) +

  geom_errorbarh(aes(xmin = local_data[local_data$local_effect == "bottom" , ]$lower,
                     xmax = local_data[local_data$local_effect == "bottom" , ]$upper)) +
  ...
  # Add axis labels
  xlab("Gain local bottom") +
  ylab("Gain local top") +

  # Add title
  ggtitle("Relative Strength Local Effects in Vertical Hemifields") +
...
# # save ggplot
# ggsave(filename = "Local TOP vs. BOTTOM with CIs.png",
#        plot = last_plot(), path = "[file path here]")
```


### Global (self) top vs. Global (self) bottom *with CIs* [NEW!]

Organize data frame.
```{r}
global_self_data <- tibble(subject = numeric(0), global_effect = character(0), lower = numeric(0), median = numeric(0), upper = numeric(0))
...
for(i in unique(bootstrap_data$subject)) {
  ...
  top = as.vector(Full.bottom - Bottom.bottom, mode = 'numeric')
  top.lower = quantile(top, 0.025)
  top.median = quantile(top, 0.5)
  top.upper = quantile(top, 0.975)
  
  global_self_data %<>%
    add_row(subject = i, global_effect = "top", lower = top.lower, median = top.median, upper = top.upper)
  ...
  bottom = as.vector(Full.top - Top.top, mode = 'numeric')
  bottom.lower = quantile(bottom, 0.025)
  bottom.median = quantile(bottom, 0.5)
  bottom.upper = quantile(bottom, 0.975)
  
  global_self_data %<>%
    add_row(subject = i, global_effect = "bottom", lower = bottom.lower, median = bottom.median, upper = bottom.upper)
}
```

Plotting
```{r}
Global <- tibble(.rows = n_subjects) # empty table n_subjects x n_conditions

Global %<>%
  mutate(
    Global_top = global_self_data[global_self_data$global_effect == "top", ]$median,
    Global_bottom = global_self_data[global_self_data$global_effect == "bottom", ]$median,
    
    # retain info from local_data dataframe
    subject = global_self_data[global_self_data$global_effect == "top", ]$subject
  )

Global %>%

  ggplot(aes(x = Global_bottom, y = Global_top, label = subject)) +
  ...
  # ADDED
  geom_text(aes(label = subject, hjust = 1, vjust = 3), check_overlap = FALSE) +

  geom_errorbar(aes(ymin = global_self_data[global_self_data$global_effect == "top" , ]$lower,
                    ymax = global_self_data[global_self_data$global_effect == "top" , ]$upper)) +

  geom_errorbarh(aes(xmin = global_self_data[global_self_data$global_effect == "bottom" , ]$lower,
                     xmax = global_self_data[global_self_data$global_effect == "bottom" , ]$upper)) +
  ...
  # Add axis labels
  xlab(TeX("Gain global_{self} bottom")) +
  ylab(TeX("Gain global_{self} top")) +

  # Add title
  ggtitle(TeX("Relative Strength of Global_{self} Effects in Vertical Hemifields")) +
...
# # save ggplot
# ggsave(filename = "Global self TOP vs. BOTTOM with CIs.png",
#        plot = last_plot(), path = "[file path here]")
```


### Global (obj) top vs. Global (obj) bottom *with CIs* [NEW!]

Organize data frame.
```{r}
global_obj_data <- tibble(subject = numeric(0), global_effect = character(0), lower = numeric(0), median = numeric(0), upper = numeric(0))
...
for(i in unique(bootstrap_data$subject)) {
  ...
  Control.bottom = bootstrap_data %>%
    filter(subject == i, condition == "Control", probe_index == "bottom") %>%
    subset(select = c(7:1006))
  ...
  Control.top = bootstrap_data %>%
    filter(subject == i, condition == "Control", probe_index == "top") %>%
    subset(select = c(7:1006))
  
  # Global (obj) top
  top = as.vector((Bottom.top - Control.top) - (Full.top - Top.top), mode = 'numeric')
  top.lower = quantile(top, 0.025)
  top.median = quantile(top, 0.5)
  top.upper = quantile(top, 0.975)
  ...
  # Global (obj) bottom
  bottom = as.vector((Top.bottom - Control.bottom) - (Full.bottom - Bottom.bottom), mode = 'numeric')
  bottom.lower = quantile(bottom, 0.025)
  bottom.median = quantile(bottom, 0.5)
  bottom.upper = quantile(bottom, 0.975)
...
Global %<>%
  mutate(
    Global_top = global_obj_data[global_obj_data$global_effect == "top", ]$median,
    Global_bottom = global_obj_data[global_obj_data$global_effect == "bottom", ]$median,
    
    # retain info from local_data dataframe
    subject = global_obj_data[global_obj_data$global_effect == "top", ]$subject
  )
...
Global %>%
  ...
  geom_errorbar(aes(ymin = global_obj_data[global_obj_data$global_effect == "top" , ]$lower,
                    ymax = global_obj_data[global_obj_data$global_effect == "top" , ]$upper)) +

  geom_errorbarh(aes(xmin = global_obj_data[global_obj_data$global_effect == "bottom" , ]$lower,
                     xmax = global_obj_data[global_obj_data$global_effect == "bottom" , ]$upper)) +
  ...
  # Add axis labels
  xlab(TeX("Gain global_{obj} bottom")) +
  ylab(TeX("Gain global_{obj} top")) +

  # Add title
  ggtitle(TeX("Relative Strength of Global_{obj} Effects in Vertical Hemifields")) +
...
# # save ggplot
# ggsave(filename = "Global obj TOP vs. BOTTOM with CIs.png",
#        plot = last_plot(), path = "[file path here]")
```

### Control-top vs. Control-bottom *with CIs* [NEW!]
```{r}
Control <- tibble(.rows = n_subjects) # empty table n_subjects x n_conditions

Control %<>%
  mutate(
    Control.top = 
      CI[CI$condition == "Control" & CI$probe_index == "top", ]$median_alpha,
    
    Control.bottom =
      CI[CI$condition == "Control" & CI$probe_index == "bottom", ]$median_alpha,
    
    # retain info from CI dataframe
    subject = CI[(CI$condition == "Control" & CI$probe_index == "top"), ]$subject
  )

Control %>%
  
  ggplot(aes(x = Control.bottom, y = Control.top, label = subject)) +
  
  geom_abline() + # identity line plotted *first*
  geom_hline(yintercept = 0, lty = 2) +
  geom_vline(xintercept = 0, lty = 2) +
  
  geom_point(colour = Control$subject, size = 2) +
    
  # ADDED
  geom_text(aes(label = subject, hjust = 1.5, vjust = -1), check_overlap = FALSE) +
  
  geom_errorbar(aes(ymin = CI[CI$condition == "Control" & CI$probe_index == "top", ]$lower,
                    ymax = CI[CI$condition == "Control" & CI$probe_index == "top", ]$upper)) +
  
  geom_errorbarh(aes(xmin = CI[CI$condition == "Control" & CI$probe_index == "bottom", ]$lower,
                     xmax = CI[CI$condition == "Control" & CI$probe_index == "bottom", ]$upper)) +
  ...
  # Add axis labels
  xlab(TeX("Gain F_{Control} P_{Bottom}")) +    
  ylab(TeX("Gain F_{Control} P_{Top}")) +
...
# # save ggplot
# ggsave(filename = "Control-top vs. Control-bottom with CIs.png",
#        plot = last_plot(), path = "[file path here]")
```
...
Maximum gain for Control condition.
```{r}
data_Control <- data[data$condition == "Control", ]

data_Control %>%
  filter(alpha == max(data_Control$alpha)) %>% # subject with max gain

  ggplot(aes(x = abs_tilt, y = probe_angle)) +
  ...
  xlab("Absolute tilt (deg)") +    
  ylab("Reported tilt (deg)") +

  # Add title
  ggtitle("Reported vs. Actual direction of motion", subtitle = paste(paste("Subject", unique(data_Control[data_Control$alpha == max(data_Control$alpha), ]$subject),
                                                                      paste("Probe ", unique(data_Control[data_Control$alpha == max(data_Control$alpha), ]$probe_index))))) +
...
data_raw_subject %>%
  ...
  filter(subject == select_subject, condition == select_cond) %>%
  ...
  # Add axis labels
  xlab("Actual Probe Direction (deg)") +    
  ylab("Perceived Probe direction (deg)") +
  
  # Add title
  ggtitle("Perceived vs. Actual direction of motion", 
          subtitle = paste(paste(paste(paste("Subject", select_subject), select_cond, sep = ", ")), "optic flow")) +
...
## Prediction plot (N=1) - raw data
```{r}
data %>%
  
  filter(subject == select_subject, condition == select_cond, probe_index == select_probe) %>%
  
  # group_by(condition, probe_index) %>%
  ggplot(aes(x = probe_angle, y = abs_tilt)) + # shape = 
  
  geom_point(color = "grey40", alpha = .4) +
  ...
  # Add title
  ggtitle("Perceived vs. Actual direction of motion", 
          subtitle = paste(paste(paste(paste(paste(paste("Subject", select_subject), select_cond, sep = ", ")), "optic flow"), "probe", sep = ", "), select_probe)) +
...
# # save ggplot
# ggsave(filename = paste(paste("1 Subject", select_subject), ".png", sep = ""),
#        plot = last_plot(), path = "[file path here]")
```

## Prediction plots (N=1) - raw data + gain
```{r}
data %>%
  filter(subject == select_subject, condition == select_cond, probe_index == select_probe) %>%
...
# # save ggplot
# ggsave(filename = paste(paste("2 Subject", select_subject), ".png", sep = ""),
#        plot = last_plot(), path = "[file path here]")
```

## Prediction plots (N=1) - raw data + 3 gains
```{r}
data %>%
  ...
  geom_point(aes(y = predicted_absolute_tilt, colour = "best fitting gain")) +
  ...
  geom_point(alpha = .4) + # raw data points plotted *last*
  ...
  scale_color_manual(
    name = "Raw data & Gain",
    breaks = c("data", "best fitting gain", "gain = 0", "gain = 1"),
    values = c("grey50", "red", "skyblue2", "green2"),
    drop = TRUE
  ) +
...
# # save ggplot
# ggsave(filename = paste(paste("3 Subject", select_subject), ".png", sep = ""),
#        plot = last_plot(), path = "[file path here]")
```

## Prediction plots (N=1) - raw data + multiple gains
```{r}
data %>%
  ...
  geom_point(alpha = .4) +
  geom_point(aes(y = predicted_absolute_tilt, colour = "best fitting gain"), size = 0.7) +
  ...
  geom_point(aes(y = 
                    predicted_absolute_tilt(
                      ...
                      flow.x,
                      flow.y,
                      alpha = 0), colour = "gain = 0"), size = 0.7) +
  ...
  geom_point(aes(y = 
                    predicted_absolute_tilt(
                      ...
                      flow.x,
                      flow.y,
                      alpha = 1), colour = "gain = 1"), size = 0.7) +
  ...
  geom_point(aes(y = 
                   predicted_absolute_tilt(
                     ...
                     flow.x,
                     flow.y,
                     alpha = 0.2), colour = "gain = 0.2"), size = 0.7) +
  ...
  geom_point(aes(y = 
                   predicted_absolute_tilt(
                     ...
                     flow.x,
                     flow.y,
                     alpha = 0.5), colour = "gain = 0.5"), size = 0.7) +
  ...
  geom_point(aes(y = 
                   predicted_absolute_tilt(
                     ...
                     flow.x,
                     flow.y,
                     alpha = 0.6), colour = "gain = 0.6"), size = 0.7) +
  ...
  geom_point(aes(y = 
                   predicted_absolute_tilt(
                     ...
                     flow.x,
                     flow.y,
                     alpha = 0.8), colour = "gain = 0.8"), size = 0.7) +
  ...
  scale_color_manual(
    name = "Raw data & Gain",
    breaks = c("data", "best fitting gain", "gain = 0", "gain = 1", "gain = 0.2", "gain = 0.5", "gain = 0.6", "gain = 0.8"),
    values = c("grey50", "red", "skyblue2", "green2", "hotpink1", "darkorchid3", "orange1", "sienna3"),
    drop = TRUE
  ) +
...
# # save ggplot
# ggsave(filename = paste(paste("4 Subject", select_subject), ".png", sep = ""),
#        plot = last_plot(), path = "[file path here]")
```
