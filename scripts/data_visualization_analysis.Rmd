...
Same.Opposite %>%
  ...
  # Add title
  ggtitle("Accuracy of Report based on Optic flow/Probe Location", subtitle = paste("with bootstrapping. N =", n_subjects)) +
...
top.bottom %>%
  ...
  # Add title
  ggtitle("Relative Strength of Local Mechanisms in Vertical Hemifields", subtitle = paste("with bootstrapping. N =", n_subjects)) +
...
Same.comparison %>%
  ...
  # Add title
  ggtitle("Accuracy of Report for Same Flow/probe Locations", subtitle = paste("with bootstrapping. N =", n_subjects)) +
...
Opposite %>%
  ...
  # Add title
  ggtitle("Accuracy of Report for Opposite Flow/probe Locations", subtitle = paste("with bootstrapping. N =", n_subjects)) +
...
Local %>%
  ...
  # Add axis labels
  xlab("Gain (Bottom-bottom - Bottom-top)") +
  ylab("Gain (Top-top - Top-bottom)") +

  # Add title
  ggtitle("Strength of Local Mechanisms in Vertical Hemifields", subtitle = paste("with bootstrapping. N =", n_subjects)) +
...
### Global top vs. Global bottom *with CIs* [NEW!]

Organize data frame.
```{r}
global_data <- tibble(subject = numeric(0), global_effect = character(0), lower = numeric(0), median = numeric(0), upper = numeric(0))
...
for(i in unique(bootstrap_data$subject)) {
  ...
  top = as.vector(Full.top - Top.top, mode = 'numeric')
  top.lower = quantile(top, 0.025)
  top.median = quantile(top, 0.5)
  top.upper = quantile(top, 0.975)
  ...
  bottom = as.vector(Full.bottom - Bottom.bottom, mode = 'numeric')
  bottom.lower = quantile(bottom, 0.025)
  bottom.median = quantile(bottom, 0.5)
  bottom.upper = quantile(bottom, 0.975)
...
Global %<>%
  mutate(
    Global_top = global_data[global_data$global_effect == "top", ]$median,
    Global_bottom = global_data[global_data$global_effect == "bottom", ]$median,
    
    # retain info from local_data dataframe
    subject = global_data[global_data$global_effect == "top", ]$subject
  )
...
Global %>%
  ...
  geom_errorbar(aes(ymin = global_data[global_data$global_effect == "top" , ]$lower,
                    ymax = global_data[global_data$global_effect == "top" , ]$upper)) +

  geom_errorbarh(aes(xmin = global_data[global_data$global_effect == "bottom" , ]$lower,
                     xmax = global_data[global_data$global_effect == "bottom" , ]$upper)) +
  ...
  # Add axis labels
  xlab("Gain (Full-bottom - Bottom-bottom)") +
  ylab("Gain (Full-top - Top-top)") +

  # Add title
  ggtitle("Strength of Global Mechanisms in Vertical Hemifields", subtitle = paste("with bootstrapping. N =", n_subjects)) +
...
# # save ggplot
# ggsave(filename = "Global TOP vs. BOTTOM with CIs.png",
#        plot = last_plot(), path = "[file path here]")
```
...
Control %>%
  ...
  geom_point(colour = Control$subject, size = 3) +
  ...
  # Add title
  ggtitle("Accuracy of Report with No Optic Flow", subtitle = paste("with bootstrapping. N =", n_subjects)) +
